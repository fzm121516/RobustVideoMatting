...
for i in range(num_video):
    ...
    if len(parts) == 4:  # 如果分割后的部分数量为4，说明文件名格式正确
        subdir = os.path.join(parts[3])  # 合并第二部分和第三部分
    else:  # 如果文件名格式不符合预期，跳过该文件
        print(f"Unexpected filename format: {image_name}")
        continue

    # 保存结果
    output_dir = os.path.join(args.result_dir, subdir)  # 生成输出目录路径
    if not os.path.exists(output_dir):  # 如果输出目录不存在，则创建目录
        os.makedirs(output_dir)

    # 生成保存路径
    save_path = os.path.join(output_dir, video_name)  # 拼接保存路径
